...
class ControllerExtensionImportseo extends Controller { 
	...
	public function importproduct() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'extension/importseo')) {
	        if (is_uploaded_file($this->request->files['importseoproduct']['tmp_name'])) {
	            $filePath = $this->request->files['importseoproduct']['tmp_name'];
	        } else {
	        ...
	        try {
	            ...
	            $this->session->data['warning'] = 'Error loading file: ' . $e->getMessage();
	            $this->response->redirect($this->url->link('extension/importseo', 'user_token=' . $this->session->data['user_token'], true));
	            return;
	        }
	        ...
	        $this->session->data['success'] = 'Total SEO URL Edited: ' . $totalnewproduct;
	        $this->response->redirect($this->url->link('extension/importseo', 'user_token=' . $this->session->data['user_token'], true));
	    }
	}
	public function importcategory(){
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'extension/importseo')) {
			...
			if ($content) {
				...
				try {
					$objPHPExcel = PHPExcel_IOFactory::load($this->request->files['importseocategory']['tmp_name']);
				} catch(Exception $e) {
	...
	public function importinfo() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'extension/importseo')) {
	        if (is_uploaded_file($this->request->files['importinformation']['tmp_name'])) {
	            $filePath = $this->request->files['importinformation']['tmp_name'];
	            ...
	            try {
	                ...
	                $this->session->data['success'] = 'Total SEO URL Edited: ' . $totalnewinformation;
	            } catch (Exception $e) {
	                $this->log->write('Error processing file: ' . $e->getMessage());
	                $this->session->data['warning'] = 'Error processing file. Please check the file format and content.';
	            }
	        } else {
	            $this->session->data['warning'] = $this->language->get('error_empty');
	        }
	...
	public function importmanuf() {
	    ...
	    if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->user->hasPermission('modify', 'extension/importseo')) {
	        if (isset($this->request->files['importmanufac']['tmp_name']) && is_uploaded_file($this->request->files['importmanufac']['tmp_name'])) {
	            $filePath = $this->request->files['importmanufac']['tmp_name'];
	            ...
	            try {
	                ...
	                $this->session->data['success'] = 'Total SEO URL Edited: ' . $totalnewmanufacturer;
	            } catch (Exception $e) {
	                $this->log->write('Error processing file: ' . $e->getMessage());
	                $this->session->data['warning'] = 'Error processing file. Please check the file format and content.';
	            }
	...
	public function exportproduct() {
	    ...
	    $this->load->model('localisation/language');
	    require_once (DIR_SYSTEM . '/library/tmd/PHPExcel.php'); // Adjust this path if necessary
	    $objPHPExcel = new PHPExcel(); // Correct instantiation
	    $i = 1;

	    // Set header values
	    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$i, 'Product ID');
	    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$i, 'Model');
	    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$i, 'Language Code');
	    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$i, 'Keyword');
	    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$i, 'Name');
	    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$i, 'Manufacturer');
	    $i++;

	    // SQL query to fetch products
	    if (!empty($category_id)) {
	        $sql = "SELECT p.product_id, p.model, p.manufacturer_id FROM `".DB_PREFIX."product` p LEFT JOIN `".DB_PREFIX."product_to_category` pc ON pc.product_id = p.product_id WHERE pc.category_id = '" . (int)$category_id . "'";
	    } else {
	    ...
	    if ($query->num_rows) { // Use num_rows to check if there are results
	        foreach ($query->rows as $rowproduct) {
	            ...
	            if (!empty($language_id)) {
	                $sql1 .= " AND language_id='" . (int)$language_id . "'";
	            }
	            $query1 = $this->db->query($sql1);

	            if ($query1->num_rows) { // Check if there are results
	                foreach ($query1->rows as $row) {
	                    $sql2 = "SELECT name FROM `".DB_PREFIX."product_description` WHERE product_id='" . (int)$rowproduct['product_id'] . "' AND language_id='" . (int)$row['language_id'] . "'";
	                    $query2 = $this->db->query($sql2);

	                    $sql3 = "SELECT name FROM `".DB_PREFIX."manufacturer` WHERE manufacturer_id='" . (int)$rowproduct['manufacturer_id'] . "'";
	                    ...
	                    // Set values in objPHPExcel
	                    $objPHPExcel->getActiveSheet()->SetCellValue('A'.$i, $rowproduct['product_id']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('B'.$i, $rowproduct['model']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('C'.$i, $language_code);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('D'.$i, $row['keyword']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('E'.$i, isset($query2->row['name']) ? $query2->row['name'] : '');
	                    $objPHPExcel->getActiveSheet()->SetCellValue('F'.$i, $manufacturer);
	                    $i++;
	                }
	            } else {
	                // Handle case where no SEO URL is found
	                $sql2 = "SELECT name FROM `".DB_PREFIX."product_description` WHERE product_id='" . (int)$rowproduct['product_id'] . "' AND language_id='" . (int)$this->config->get('config_language_id') . "'";
	                $query2 = $this->db->query($sql2);
	                $sql3 = "SELECT name FROM `".DB_PREFIX."manufacturer` WHERE manufacturer_id='" . (int)$rowproduct['manufacturer_id'] . "'";
	                $query3 = $this->db->query($sql3);
	    ...
	    $objPHPExcel->getActiveSheet()->getStyle('A1:F1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FF4F81BD');
	    ...
	    // Output to browser
	    $filename = 'SeoProduct-Export-' . time() . '.xls'; // Keep as .xls
	    $objPHPExcel->getActiveSheet()->setTitle('All product');
	    $writer = new PHPExcel_Writer_Excel5($objPHPExcel); // Use the correct writer for .xls
	    header('Content-Type: application/vnd.ms-excel'); // Correct content type for .xls
	...
	public function exportcategory() {
	    ...
	    // Ensure PHPExcel is included
	    require_once (DIR_SYSTEM . '/library/tmd/PHPExcel.php'); // Adjust this path if necessary
	    $objPHPExcel = new PHPExcel(); // Correct instantiation

	    $i = 1;
	    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $i, 'Category Id');
	    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $i, 'Language Code');
	    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $i, 'Keyword');
	        ...
	        foreach ($query->rows as $rowproduct) {
	            ...
	            if ($query1->num_rows) { // Check if there are results
	                foreach ($query1->rows as $row) {
	                    ...
	                    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $i, $rowproduct['category_id']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $i, $language_code);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $i, $row['keyword']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $i, $query2->row['name']);
	                    $i++;
	                }
	            } else {
	    ...
	    // Output to browser
	    $filename = 'SeoCategory-Export-' . time() . '.xls'; // Keep as .xls
	    $objPHPExcel->getActiveSheet()->setTitle('All product');
	    $writer = new PHPExcel_Writer_Excel5($objPHPExcel); // Use the correct writer for .xls
	    header('Content-Type: application/vnd.ms-excel'); // Correct content type for .xls
	    header('Content-Disposition: attachment; filename="' . urlencode($filename) . '"');
	...
	public function exportinfo() {
	    ...
	    // Set header values
	    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $i, 'Information Id');
	    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $i, 'Language Code');
	    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $i, 'Keyword');
	    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $i, 'Name');
	    $i++;
	        ...
	        foreach ($query->rows as $rowproduct) {
	            ...
	            if ($query1->num_rows) { // Check if there are results
	                foreach ($query1->rows as $row) {
	                    ...
	                    // Set values in objPHPExcel
	                    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $i, $rowproduct['information_id']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $i, $language_code);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $i, $row['keyword']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $i, isset($query2->row['title']) ? $query2->row['title'] : '');
	                    $i++;
	                }
	            } else {
	                // Handle case where no SEO URL is found
	    ...
	    $objPHPExcel->getActiveSheet()->getRowDimension(1)->setRowHeight(30);
	    $objPHPExcel->getActiveSheet()->getStyle('A1:D1')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('FF4F81BD');
	    ...
	    // Output to browser
	    $filename = 'SeoInformation-Export-' . time() . '.xls'; // Keep as .xls
	    $objPHPExcel->getActiveSheet()->setTitle('All Information');
	    $writer = new PHPExcel_Writer_Excel5($objPHPExcel); // Use the correct writer for .xls
	    header('Content-Type: application/vnd.ms-excel'); // Correct content type for .xls
	    header('Content-Disposition: attachment; filename="' . urlencode($filename) . '"');
	    $writer->save('php://output');
	...
	public function exportmanuf() {
	    ...
	    // Set header values
	    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $i, 'Manufacturer Id');
	    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $i, 'Language Code');
	    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $i, 'Keyword');
	    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $i, 'Name');
	    $i++;

	    // SQL query to fetch manufacturers
	    ...
	    if ($query->num_rows) { // Use num_rows to check if there are results
	        foreach ($query->rows as $rowproduct) {
	            ...
	            if (!empty($language_id)) {
	                $sql1 .= " AND language_id='" . $language_id . "'"; // Ensure space before AND
	            }
	            $query1 = $this->db->query($sql1);
	            ...
	            if ($query1->num_rows) { // Check if there are results
	                foreach ($query1->rows as $row) {
	                    ...
	                    // Set values in objPHPExcel
	                    $objPHPExcel->getActiveSheet()->SetCellValue('A' . $i, $rowproduct['manufacturer_id']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('B' . $i, $language_code);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('C' . $i, $row['keyword']);
	                    $objPHPExcel->getActiveSheet()->SetCellValue('D' . $i, $rowproduct['name']);
	                    $i++;
	                }
	            } else {
	                // Handle case where no SEO URL is found
	    ...
	    // Output to browser
	    $filename = 'SeoManufacturer-Export-' . time() . '.xls'; // Keep as .xls
	    $objPHPExcel->getActiveSheet()->setTitle('All Manufacturers');
	    $writer = new PHPExcel_Writer_Excel5($objPHPExcel); // Use the correct writer for .xls
	    header('Content-Type: application/vnd.ms-excel'); // Correct content type for .xls
	    header('Content-Disposition: attachment; filename="' . urlencode($filename) . '"');
	    $writer->save('php://output');
